...
${pc.bold('Install Commands')} (no Bun required):
  ...
  ${pc.cyan('npx claude-mem install --provider claude|gemini|openrouter|ollama|deepseek')}   Set LLM provider non-interactively
...
function parseInstallOptions(argv: string[]): InstallOptions {
  ...
  if (provider !== undefined && provider !== 'claude' && provider !== 'gemini' && provider !== 'openrouter' && provider !== 'ollama' && provider !== 'deepseek') {
    console.error(`Unknown --provider: ${provider}. Allowed: claude, gemini, openrouter, ollama, deepseek`);
    process.exit(1);
  }
  return {
